...
if (Test-Path $file){
  Write-Output "Found $file already downloaded locally."
} Else {
...
if ($unzipMethod -eq '7zip') {
  # download 7zip
  if (Test-Path $7zaExe) {
    Write-Output "Found $7zaExe already downloaded locally."
  } else {
  ...
  #On a Windows "Core Server", the Windows Shell
  #does not exist, so use .net 4.5 for unzipping...
  ...
  #The following code depends on the Windows Shell...
